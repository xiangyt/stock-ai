...
func (s *DataCollectService) CollectPerformanceReports(sourceName, code string) (*CollectResult, error) {
	...
	result := s.upsertPerformanceReports(code, reports)
...
func (s *DataCollectService) CollectPerformanceReportsBatch(sourceName string) (*CollectResult, error) {
	...
	for i, stock := range stocks {
		...
		partial := s.upsertPerformanceReports(stock.Code, reports)
		result.NewCount += partial.NewCount
		result.UpdCount += partial.UpdCount
...
func (s *DataCollectService) CollectShareholderCounts(sourceName, code string) (*CollectResult, error) {
	...
	result := s.upsertShareholderCounts(code, counts)
...
func (s *DataCollectService) CollectShareholderCountsBatch(sourceName string) (*CollectResult, error) {
	...
	for i, stock := range stocks {
		...
		partial := s.upsertShareholderCounts(stock.Code, counts)
		result.NewCount += partial.NewCount
		result.UpdCount += partial.UpdCount
...
func (s *DataCollectService) CollectShareChanges(sourceName, code string) (*CollectResult, error) {
	...
	result := s.upsertShareChanges(code, changes)
...
func (s *DataCollectService) CollectShareChangesBatch(sourceName string) (*CollectResult, error) {
	...
	for i, stock := range stocks {
		...
		partial := s.upsertShareChanges(stock.Code, changes)
		result.NewCount += partial.NewCount
		result.UpdCount += partial.UpdCount
...
func (s *DataCollectService) loadAllStockCodes() []model.Stock {
	var stocks []model.Stock
	s.db.Select("code").Find(&stocks)
	return stocks
}

// upsertPerformanceReports 批量写入财报数据
func (s *DataCollectService) upsertPerformanceReports(code string, reports []adapter.PerformanceReport) *CollectResult {
	...
	for _, r := range reports {
		...
		rowsAffected := s.db.Clauses(clause.OnConflict{
			Columns:   []clause.Column{{Name: "stock_code"}, {Name: "report_date"}},
			DoUpdates: clause.AssignmentColumns(updateCols),
		}).Create(&m)

		if rowsAffected.Error != nil {
			log.Printf("[采集-财报] upsert失败 [%s/%s]: %v", code, r.ReportDate, rowsAffected.Error)
			continue
		}
...
func (s *DataCollectService) upsertShareholderCounts(code string, counts []adapter.ShareholderCount) *CollectResult {
	...
	for _, c := range counts {
		...
		if rowsAffected.Error != nil {
			log.Printf("[采集-股东户数] upsert失败 [%s/%s]: %v", code, c.EndDate, rowsAffected.Error)
			continue
		}
		if rowsAffected.RowsAffected == 0 {
			result.NewCount++
...
func (s *DataCollectService) upsertShareChanges(code string, changes []adapter.ShareChange) *CollectResult {
	...
	for _, c := range changes {
		...
		if rowsAffected.Error != nil {
			log.Printf("[采集-股本变动] upsert失败 [%s/%s]: %v", code, c.Date, rowsAffected.Error)
			continue
		}
		if rowsAffected.RowsAffected == 0 {
			result.NewCount++
